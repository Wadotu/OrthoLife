...
public class OrthoCalendar {
    ...
    private static final List<String> READING_PRIORITY =
            Arrays.asList("epistle", "apostle", "acts", "gospel");

    public static OrthoDay.Reading getBestReading(List<OrthoDay.Reading> readings) {
        if (readings == null || readings.isEmpty()) return null;
        OrthoDay.Reading best = null;
        int bestIdx = Integer.MAX_VALUE;
        for (OrthoDay.Reading r : readings) {
            String desc = r.description != null ? r.description.toLowerCase(Locale.ENGLISH) : "";
            int idx = Integer.MAX_VALUE;
            for (int i = 0; i < READING_PRIORITY.size(); i++) {
                if (desc.contains(READING_PRIORITY.get(i))) { idx = i; break; }
            }
            if (idx < bestIdx) { bestIdx = idx; best = r; }
        }
        return best != null ? best : readings.get(0);
    }
    ...
    public static String translateReadingToKorean(String display) {
        if (display == null) return "";
        String cleanDisplay = display;
        if (display.toLowerCase().startsWith("the ")) {
            cleanDisplay = display.substring(4);
        }
        
        // 정교회 성경 봉독에서 자주 쓰이는 영문 도서명 -> canonical 매핑
        String[] enBooks = {
            "Genesis", "Exodus", "Leviticus", "Numbers", "Deuteronomy", "Joshua", "Judges", "Ruth",
            "1 Samuel", "I Samuel", "2 Samuel", "II Samuel", "1 Kings", "I Kings", "2 Kings", "II Kings",
            "1 Chronicles", "I Chronicles", "2 Chronicles", "II Chronicles", "Ezra", "Nehemiah", "Tobit", "Judith", "Esther",
            "1 Maccabees", "I Maccabees", "2 Maccabees", "II Maccabees", "3 Maccabees", "III Maccabees",
            "Job", "Psalms", "Psalm", "Proverbs", "Ecclesiastes", "Song of Solomon", "Wisdom of Solomon", "Wisdom", "Sirach", "Ecclesiasticus",
            ...
            "Matthew", "Mark", "Luke", "John", "Acts", "Romans", "1 Corinthians", "I Corinthians", "2 Corinthians", "II Corinthians", "Galatians", "Ephesians", "Philippians", "Colossians",
            "1 Thessalonians", "I Thessalonians", "2 Thessalonians", "II Thessalonians", "1 Timothy", "I Timothy", "2 Timothy", "II Timothy", "Titus", "Philemon", "Hebrews", "James",
            "1 Peter", "I Peter", "2 Peter", "II Peter", "1 John", "I John", "2 John", "II John", "3 John", "III John", "Jude", "Revelation"
        };
        
        String[] canonicals = {
            "Gen", "Exo", "Lev", "Num", "Deu", "Jos", "Jdg", "Rut",
            "1Sa", "1Sa", "2Sa", "2Sa", "1Ki", "1Ki", "2Ki", "2Ki",
            "1Ch", "1Ch", "2Ch", "2Ch", "Ezr", "Neh", "Tob", "Jdt", "Est",
            "1Ma", "1Ma", "2Ma", "2Ma", "1Ma", "1Ma",
            "Job", "Psa", "Psa", "Pro", "Ecc", "Sgs", "Sol", "Sol", "Sir", "Sir",
            "Isa", "Jer", "Lam", "Bar", "Eze", "Dan", "Hos", "Joe", "Amo", "Oba", "Jon", "Mic", "Nah", "Hab", "Zep", "Hag", "Zec", "Mal",
            "Mat", "Mar", "Luk", "Joh", "Act", "Rom", "1Co", "1Co", "2Co", "2Co", "Gal", "Eph", "Phi", "Col",
            "1Th", "1Th", "2Th", "2Th", "1Ti", "1Ti", "2Ti", "2Ti", "Tit", "Phm", "Heb", "Jam",
            "1Pe", "1Pe", "2Pe", "2Pe", "1Jo", "1Jo", "2Jo", "2Jo", "3Jo", "3Jo", "Jud", "Rev"
        };

        for (int i = 0; i < enBooks.length; i++) {
            if (cleanDisplay.toLowerCase().startsWith(enBooks[i].toLowerCase())) {
                String koName = BookNameMapper.get(canonicals[i]);
                if (display.toLowerCase().startsWith("the ")) {
                    return koName + display.substring(4 + enBooks[i].length());
                } else {
                    return koName + display.substring(enBooks[i].length());
                }
            }
        }
        return display;
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (bestTitle.isEmpty()) {
                        ...
                        if (day.feast_level >= 7 && day.feast_level_description != null && !day.feast_level_description.isEmpty()) {
                            info.feast = bestTitle + " (" + day.feast_level_description + ")";
                        } else {
                            info.feast = bestTitle;
                        }
                    }

                    // --- [Fasting Localization Applied Here] ---
                    info.fasting = translateFasting(context, day);

                    if (day.readings != null && !day.readings.isEmpty()) {
                        List<OrthoDay.Reading> sorted = new java.util.ArrayList<>(day.readings);
                        java.util.Collections.sort(sorted, (a, b) -> {
                            ...
                            for (int i = 0; i < READING_PRIORITY.size(); i++) {
                                if (da.contains(READING_PRIORITY.get(i))) { ia = i; break; }
                            }
                            for (int i = 0; i < READING_PRIORITY.size(); i++) {
                                if (db.contains(READING_PRIORITY.get(i))) { ib = i; break; }
                            }
                        ...
                        for (OrthoDay.Reading r : sorted) {
                            String display = r.display;
                            if ("ko".equals(currentLang)) {
                                display = translateReadingToKorean(display);
                            }
                            String desc = translateReadingDescription(context, r.description);
                            if (sb.length() > 0) sb.append("\n");
                            sb.append((desc != null && !desc.isEmpty()) ? "[" + desc + "] " + display : display);
                        }
                        info.dailyVerse = sb.toString();
                    } else {
                        info.dailyVerse = "";
                    }
                    ...
                    new Handler(Looper.getMainLooper()).post(() -> callback.onDataLoaded(info));
                } else {
                    new Handler(Looper.getMainLooper()).post(() -> callback.onError("API Error"));
                }
            }
    ...
    public static void getFullCalendarDay(Context context, String apiMode, int year, int month, int day, FullCalendarCallback callback) {
        ...
        service.getDay(apiMode, year, month, day).enqueue(new Callback<OrthoDay>() {
            ...
            public void onResponse(Call<OrthoDay> call, Response<OrthoDay> response) {
                if (response.isSuccessful() && response.body() != null) {
                    new Handler(Looper.getMainLooper()).post(() -> callback.onDataLoaded(response.body()));
                } else {
    ...
    /**
     * 금식 명칭 번역 로직 (strings.xml 리소스와 매핑)
     */
    public static String translateFasting(Context context, OrthoDay day) {
        if (day == null) return "";
        
        String desc = day.fast_level_desc != null ? day.fast_level_desc.toLowerCase() : "";
        String exc = day.fast_exception_desc != null ? day.fast_exception_desc.toLowerCase() : "";
    ...
    public static String translateApiTitle(Context context, String apiTitle) {
        ...
        if ("ko".equals(currentLang)) {
            // [A 구역] 대축일 매핑
            if (normal.equals("Holy Pascha")) return context.getString(R.string.pascha_feast);
            ...
            if (normal.contains("Feast of the Holy Trinity")) return context.getString(R.string.pentecost_feast);
            ...
            if (lower.contains("dormition")) return context.getString(R.string.dormition);
            ...
            if (normal.contains("Day of the Holy Spirit")) return "성령 축일";
            ...
            if (lower.contains("fathers of the 1st council")) return "제1차 세계 공의회 참석 교부들 주일";
            ...
            if (lower.contains("dormition") && lower.contains("theotokos")) return "성모님의 안식";
            ...
            if (lower.contains("nativity") && lower.contains("theotokos")) return "성모탄생 축일";
            ...
            if (lower.contains("menas") && lower.contains("egypt")) return "성 미나스 대순교자";
            if (lower.contains("john") && lower.contains("merciful")) return "성 요한 알렉산드리아의 총대주교";
            if (lower.contains("chrysostom")) return "성 요한 크리소스톰 콘스탄티노플의 총대주교";
            ...
            if (lower.contains("catherine") && lower.contains("great")) return "성 까떼리나 대순교자";
            ...
            if (lower.contains("nicholas") && lower.contains("myra")) return "성 니콜라스 미라의 대주교";
            ...
            if (lower.contains("spyridon") && lower.contains("trimythus")) return "성 스피리돈 대주교";
            ...
            if (lower.contains("kallinikos")) return "성 갈리니꼬스 순교자";
